...
public class ConditioningData : MonoBehaviour
{
    private FileSystemWatcher watcher;
    private string folderPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "AppData", "ConditioningData");
    // Start is called before the first frame update
    void Start()
    {
        InitializeConditioningFileWatcher();
    }
    private void InitializeConditioningFileWatcher()
    {
        try
        {
            ...
            Debug.Log("Conditioning Folder Found");
        }
        catch (Exception e)
        {
    ...
    private void OnConditioningFileCreated(object sender, FileSystemEventArgs e)
    {
        ...
        while (!isFileReady && retryCount < maxRetries)
        {
            try
            {
                ...
                {
                    Debug.Log("Conditioning檔案正在被使用，等待...");
